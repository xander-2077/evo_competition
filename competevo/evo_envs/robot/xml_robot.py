...
class Robot:
    ...
    def get_params(self, get_name: object = False) -> object:
        param_list = []
        for body in self.bodies:
...
if __name__ == "__main__":
    ...
    model_name = 'ant'
    # cfg_path = f'/root/ws/competevo/competevo/evo_envs/assets/ant.yml'
    # # model = load_model_from_path(f'assets/mujoco_envs/{model_name}.xml')
    # yml = yaml.safe_load(open(cfg_path, 'r'))
    # cfg = yml['robot']
    xml_robot = Robot(cfg, xml=f'/root/ws/competevo/competevo/evo_envs/assets/evo_ant_body.xml')
