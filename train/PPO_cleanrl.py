...
class FirstItemWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        return state, reward, any(terminated), truncated, info[0]
...
#     def __init__(self, env):
#         super().__init__(env)
#         self.action_space = gym.spaces.Box(low=-1.0, high=1.0, shape=(16,))

#     def action(self, action):
#         action = (action[:8], action[8:])  # split actions for each agent
#         return action
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_mean = nn.Sequential(
            ...
            layer_init(nn.Linear(64, 64)),
            nn.Tanh(),
            layer_init(nn.Linear(64, np.prod(envs.single_action_space[0].shape)), std=0.01),
        )
        self.actor_logstd = nn.Parameter(torch.zeros(1, np.prod(envs.single_action_space[0].shape)))
...
if __name__ == "__main__":
    ...
    # assert isinstance(envs.single_action_space[0], gym.spaces.Box), "only continuous action space is supported"
    ...
    actions = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space[0].shape).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_actions = actions.reshape((-1,) + envs.single_action_space[0].shape)
